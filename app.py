...
if st.button("🚀 Bắt đầu quét dữ liệu"):
    with st.spinner("Đang kết nối dữ liệu sàn chứng khoán..."):
        try:
            ...
            st.error("Không thể kết nối đến nguồn dữ liệu. Vui lòng thử lại sau ít phút.")
            st.stop()
        ...
        for ticker in symbols:
            try:
                # Sửa đổi cách gọi hàm ohlcv để phù hợp hoàn toàn với thư viện vnstock3 mới
                df = market.equity.ohlcv(symbol=ticker, start='2026-01-01', end='2026-07-09')
                
                # Kiểm tra nếu dữ liệu rỗng hoặc không có cột cần thiết
        ...
        if len(results) > 0:
            ...
            st.dataframe(res_df) # Hiển thị bảng tổng hợp trước để bạn dễ theo dõi
            
            st.divider() # Vạch kẻ ngang chia tách dòng
            ...
            st.error("Hệ thống không quét được dữ liệu của mã nào. Có thể nguồn cung cấp dữ liệu đang bảo trì, bạn hãy thử lại sau ít phút nhé.")
